class NotificationBroadcastJob < ApplicationJob
  ...
  def perform personal_message
    ...
    ActionCable.server.broadcast "notifications_#{personal_message.user.id}_channel",
      message: message, conversation_id: personal_message.conversation.id
    ActionCable.server.broadcast "notifications_#{personal_message.conversation.author.id}_channel",
     notification: render_notification(personal_message), message: message,
